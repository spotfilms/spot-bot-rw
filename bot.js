...
const token = '8059173409:AAGDCaC3YWFVARizM-shzllGyyGC0Vj8GuY'; // продакшен
// const token = '5711139754:AAHRax25IBod11sJgOXd6L_i8T_a5yRp5LY';

const groupId = '-1003136848013'; // продакшен
// const groupId = '-1003169780517';  
...
const STATES = {
  ...
  WAITING_VOICE: 'waiting_voice', // Состояние, где нужен ТОЛЬКО кружок
...
bot.command('start', async (ctx) => {
  // --- ДОБАВЛЕНИЕ: Отправка картинки из локального файла ---
  
  // Указываем путь к локальному изображению
  const imagePath = path.join(__dirname, 'promo.jpg'); 

  try {
    // Отправляем картинку, используя InputFile, БЕЗ подписи (caption)
    await ctx.replyWithPhoto(new InputFile(imagePath)); 
    
  } catch (e) {
    console.error(`❌ Ошибка при отправке локального фото (${imagePath}):`, e.message);
    // Продолжаем без изображения, если оно не найдено или не отправилось
  }

  // Сброс состояния и данных сессии
  ...
  await ctx.reply(
    'Привет! Мы ищем бибоев для художественного фильма о брейкинге! Мы ищем не актеров, а настоящих танцоров со своей энергетикой и уникальным стилем. Неважно из какого ты города и какой команды, если тебе от 12-ти до 35-ти. Утверждённые роли оплачиваются.\n\n' +
...
bot.on('message:text', async (ctx) => {
  ...
  switch (session.state) {
    ...
    case STATES.WAITING_CITY:
      ...
      await ctx.reply('Ссылки на твои любые соцсети:');
      break;
    ...
    case STATES.WAITING_VIDEO:
      if (text.toLowerCase() === 'готово') {
        ...
        await ctx.reply(
          'Отлично. Дальше нужно записать кружок. Важно произнести:\n\n' +
          '— Имя\n' +
          '— Возраст\n' +
          '— Рост\n' +
          '— Размер одежды/обуви\n' +
...
bot.on('message:video', async (ctx) => {
  ...
  if (session.state === STATES.WAITING_VIDEO) {
    ...
    // 🛑 ОТКЛОНЯЕМ, если ожидаем ТОЛЬКО кружок или голос
    await ctx.reply('❌ Пожалуйста, запишите видео-кружок');
  }
});
...
bot.on('message:document', async (ctx) => {
  ...
  if (session.state === STATES.WAITING_VIDEO && isVideo) {
    ...
    // 🛑 ОТКЛОНЯЕМ, если ожидаем ТОЛЬКО кружок или голос
    await ctx.reply('❌ Пожалуйста, запишите и пришлите видео-кружок');
  }
  
  // Добавляем обработку, чтобы не игнорировать другие документы (если это не видео)
  if (session.state === STATES.WAITING_VIDEO && !isVideo) {
      await ctx.reply('Принимаются только видео-файлы (или ссылки).');
  }
});

// Обработка видеокружков (ЭТОТ ОСТАЁТСЯ ОСНОВНЫМ)
...
// Обработка голосовых сообщений (Этот остаётся)
...
bot.on('message:photo', async (ctx) => {
  ...
  if (session.state === STATES.WAITING_PHOTO) {
    ...
    await ctx.reply(
      'Спасибо! Это общий шаг для брейкинга и твой первый шаг в кино. Мы вернемся к тебе в скором времени. Будет круто, если расскажешь про нас в сториз. Не забудь ссылку — t.me/street_cast_bot'
    );
...
// Функция отправки анкеты в группу (Без изменений)
